...
![](/assets/Snip20180621_5.png)

- 1、**从上图可以看出, Student 类继承自Person,Person 类继承自 NSObject.**

- 2、 **当通过Student类调用 Person类的类方法发时,先通过Student 类找到 Student 的元类对象(meteclass 对象) , 再通过Student 的元类对象(meteclass 对象)中的superclass 指针 找到 Person的 元类对象(metaclass 对象),然后在Person的元类对象(metaclass 对象)找到对应的类方法执行 .**
