...
- semaphore 可以用来控制一个方法,在同一个时间段内最多允许几条线程访问(即,方法访问的最大并发数),以达到数据安全的目的.
...
- 1 **什么情况使用自旋锁(osspinlock)比较划算？**<br>
(1) 预计线程等待锁的时间很短<br>
(2) 加锁的代码（临界区）经常被调用，但竞争情况很少发生<br>
(3) CPU资源不紧张多核处理器<br>
**注意:**<br>
iOS 不再推荐使用osspinlock自旋锁了, 在iOS 10 后 有个os_unfair_lock 是用来取代osspinlock的.
