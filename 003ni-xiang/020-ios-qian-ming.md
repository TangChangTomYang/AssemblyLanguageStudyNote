...
- 2、公钥密码也被称为**非对称密码（Asymmmetric Cryptography）**
- 3、在公钥密码中：<br>(1)、加密密钥，一般是公开的，因此该密钥被称为公钥（publicd key），解密密钥，由消息接收者自己保管，不能公开，因此也被称为私钥（private key）<br>(2)、公钥和私钥是一对，不能单独生成，一对公钥密钥统称为密钥对。<br>(3)、由公钥加密的密文，必须由该公钥对应的私钥才能解密。<br>(4)、由私钥加密的密文，必须由该私钥对应的公钥才能解密。
- 4、**非对称密钥行业常识：**<br>(1)、使用公钥加密过程一般被称做 加密。<br>(2)、使用私钥加密过程一般被称做 签名。
...
**结论：**<br> (1)、数字签名的作用不是保证机密性，仅仅是为了能够识别内容有没有被篡改，即数据的完整性。<br>(2）、防止消息发送者否认。
...
<br>**完整流程：**
![](/assets/Snip20180618_7.png)
<br>**简化版：**
![](/assets/Snip20180618_8.png)
...
####一、加密解密算法
- 1、**对称密码**<br>(1)、加密解密使用的是同一个密码。<br>(2)、加密解密速度快。<br>(3)、无法解决秘钥配送问题（即，密钥可能被窃取）


- 2、**公钥密码**<br>(1)、加密解密使用不同的密钥。<br>(2)、加密解密速度慢<br>(3)、可以解决密钥配送问题（谁都可以获取、得到公钥）<br>(4)、公钥加密（加密过程）私钥解密，私钥加密（签名过程）公钥解密。



- 3、**单向散列函数**<br>(1)、根据消息生成对应的固定长度的消息摘要（散列值）<br>(2)、主要用来验证数据的完整性，防止数据被篡改。


- 4、**数字签名**<br>(1)、用私钥加密散列值（消息摘要）生成密文，传送内容包明文和加密后的散列值密文。<br>(2)、主要来用保证数据的完整性，以及防止消息发送者否认。<br>(3)、确定不能保证公钥的合法性

- 5、**数字证书**<br>(1)、用CA的私钥对 公钥及公钥持有人信息进行签名。<br>(2)用CA的公钥从CA颁发的证书中获取持证人的公钥。
