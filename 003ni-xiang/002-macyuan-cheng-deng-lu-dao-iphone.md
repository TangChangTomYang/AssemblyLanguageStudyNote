...
- **查看SSH 的版本信息**，查看配置文件的protocal 字段
    ...
    ```
    cd /etc/ssh
    cat ssh_config //ssh_config 是客户端的配置文件
    ```
    - **服务端:**
    ```
    cd  /etc/ssh
    cat sshd_config //sshd_config 是服务端的配置文件
    ```  
<br>
...
##### step1: 建立安全连接过程详解
(1)在建立安全连接的过程中，**服务器会提供自己的身份证明**(如： RSA公钥等.为什么要验证公钥呢,因为 ssh root@ip地址 中的IP地址并不能唯一确定连接的服务器,网络中不同设备可能有相同ip)。
(2)如果客户端没有存储过服务器端的公钥信息（没有和该SSH服务器建立过连接，通常建立连接后会存储(存储地址:~/.ssh/known_hosts,如上图)服务端的RSA公钥），就会询问是否连接此服务器。
![](/assets/屏幕快照 2018-05-13 下午9.55.14.png)
![](/assets/屏幕快照 2018-05-13 下午9.57.05.png)
...
**补充：**    
**除了使用tcprelay.py 文件将本地端口与手机的22 端口绑定外，还有其他的工具 还可以使用 itnl 来绑定本机端口与手机的端口，命令如下：** 
