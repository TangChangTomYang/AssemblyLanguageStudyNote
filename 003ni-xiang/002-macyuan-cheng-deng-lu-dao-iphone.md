...
####2、SSH 通信之 &emsp;建立安全连接过程详解

- 在建立安全连接的过程中，服务器会提供自己的身份证明(如： RSA公钥.为什么要验证公钥呢,因为 ssh root@ip地址 中的IP地址并不能唯一确定连接的服务器,网络中不同设备可能有相同ip)。
    ![](/assets/屏幕快照 2018-05-13 下午9.55.14.png)
- 如果客户端没有存储过服务器端的公钥信息（没有和该SSH服务器建立过连接，通常建立连接后会存储(存储地址:~/.ssh/known_hosts,如上图)服务端的RSA公钥），就会询问是否连接此服务器。![](/assets/屏幕快照 2018-05-13 下午9.57.05.png)
